...
contract OnchainIPFS is ERC721, Ownable {
    ...
    function publishFile(
        ...
        if (bytes(_originCID).length > 0) {
            ...
            require(
                bytes(_originCommand).length > 0,
                "Origin command required"
            );
            require(
                bytes(_originCommandVersion).length > 0,
                "Origin command version required"
            );
        }
    ...
    function addTitle(uint256 _fileId, string memory _title) public {
        require(
            ownerOf(_fileId) == msg.sender,
            "Only the owner can add a title"
        );
    ...
    function addFilename(uint256 _fileId, string memory _filename) public {
        require(
            ownerOf(_fileId) == msg.sender,
            "Only the owner can add a filename"
        );
        require(
            bytes(files[_fileId].filename).length == 0,
    ...
    function addAuthor(uint256 _fileId, string memory _author) public {
        require(
            ownerOf(_fileId) == msg.sender,
            "Only the owner can add an author"
        );
    ...
    function getFileByFilename(string memory _filename)
        public
        view
        returns (File memory)
    {
        ...
        returns (string memory)
    {
        ...
        string memory svg = string(
            abi.encodePacked(
                ...
                "</text>",
                file.isFolder
                    ? '<text x="10" y="80" font-family="Roboto" font-size="16" fill="black">Folder: Yes</text>'
                    : "",
                "</svg>"
            )
        );
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                file.title,
                '"}, {',
                '"trait_type": "CID", "value": "',
                file.content,
                '"}, {',
                '"trait_type": "SHA3-512 CID", "value": "',
                file.sha3_512_CID,
                '"}, {',
                '"trait_type": "Origin CID", "value": "',
                file.originCID,
                '"}, {',
                '"trait_type": "Origin Command", "value": "',
                file.originCommand,
                '"}, {',
                '"trait_type": "Origin Command Version", "value": "',
                file.originCommandVersion,
                '"}, {',
                '"trait_type": "Folder", "value": "',
                file.isFolder ? "Yes" : "No",
                '"}]}'
            )
        );
    ...
    function base64(bytes memory data) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < paddingLength; i++) {
            result[result.length - paddingLength + i] = "=";
        }
    ...
    function validateSHA3_512CID(string memory cid)
        internal
        pure
        returns (bool)
    {
